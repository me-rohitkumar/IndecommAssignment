...
namespace Indecomm.BusinessLayer.CoreApiClient
{
    public partial class ApiClient
    {
        ...
        private async Task<ApiResponse<T1>> PostAsync<T1, T2>(Uri requestUrl, T2 content)
        {
            var response = await _httpClient.PostAsync(requestUrl.ToString(), CreateHttpContent<T2>(content));
            response.EnsureSuccessStatusCode();
            var data = await response.Content.ReadAsStringAsync();
            return JsonConvert.DeserializeObject<ApiResponse<T1>>(data);
        }

        private async Task<string> SaveDataAsync<T>(Uri requestUrl, PropertyModel content)
        {
